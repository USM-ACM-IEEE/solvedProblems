...
(function() {
  ...
  Morris.Bar = (function(_super) {
    ...
    Bar.prototype.hoverContentForRow = function(index) {
      ...
      if (typeof this.options.hoverCallback === 'function') {
        ...
        for (j = _i = 0, _len = _ref.length; _i < _len; j = ++_i) {
          y = _ref[j];
          content += "<div class='morris-hover-point'> <span class='label-1'><strong style='background-color: " + (this.colorFor(row, j, 'label')) + "'>\n " + this.options.labels[j] + ":\n </strong></span><span> " + (this.yLabelFormat(y)) + "\n</span></div>";
        }
      }
  ...
  Morris.Donut = (function() {
    ...
    Donut.prototype.redraw = function() {
      ...
      this.text2 = this.drawEmptyDonutLabel(cx, cy + 10, this.options.valueColor, 14, 800);
...
!function ($) {
    ...
    , eventOut
        ...
        this.enabled = true
        this.eventNamespace = 'popover' + new Date().getTime();
  ...
  , show: function() {
      ...
      if (this.hasContent() && this.enabled) {
        ...
        $.support.transition && this.$tip.hasClass('fade') ?
          ...
          complete()
          $(".popover-content select.selectify").select2();
        }
  ...
  , setContent: function(){
      ...
      if (title.length == 0) {
          $tip.find('.popover-header').addClass("empty");
      } else {
  ...
  , tip: function(){
      ...
      if (!this.$tip) {
        ...
        .delegate('[data-dismiss="popover"]', 'click.' + that.eventNamespace, $.proxy(this.hide, this))
      }
      return this.$tip
  }
  , destroy: function(){
  ...
  , clickClose: function(){
      ...
      // this hides the popover if somewhere other than the spawning element or inside the popover is clicked.
      var that = this
      if (that.tip().hasClass("in")) {
          $(document).on('click.' + that.eventNamespace, function (e) {
              if (!($(".popover").has(e.target).length || $(e.target).hasClass("popovermaker") || $(e.target).attr("data-dismiss") == "popover")) {
                  that.hide()
              }
          })
      } else if (!that.tip().hasClass("in")) {
